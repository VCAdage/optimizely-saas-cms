...
export const CardBlockComponent : CmsComponent<CardBlockDataFragment> = ({ data: { layout: imageLayout = "before", image, heading = "", subheading = "", description = {json: null}, color = "blue", link: button, icon }, inEditMode }) => {
    ...
    return (
<section className={`w-full h-full p-12 lg:p-24 rounded-[5px] @container/card ${additionalClasses.join(" ")} dark:!text-ghost-white dark:prose-h3:text-ghost-white dark:prose-h2:text-ghost-white`}>
    <div className="w-full h-full grid auto-rows-auto @[80rem]/card:items-center grid-cols-1 gap-16 @[80rem]/card:grid-cols-12">
        ...
        { (image || inEditMode) && <AnimatedImage imageLayout={ imageLayout }><CmsEditable as={Image} cmsFieldName="CardImage" className="rounded-[5px] w-full" src={ image } alt={""} width={660} height={440} /></AnimatedImage> }
    </div>
</section>);
}
